...
The original weights $b$ are replaced by the adapted weights $\tilde{b}$.
With this adaptation we get
\begin{equation}\label{eq:def_b_tilde}
u^{n+1} = u^n + \dt F \tilde{b}
\end{equation}
form \ref{eq:Combination}. 
This equation is contains $\tilde{b}$ as second unknown alongside $u^{n+1}$. 
First we have to determine the adapted weights to compute the new solution afterwards.
For the computation of the stage derivatives only the $A$ has to be known. The weights are not needed for this computation. 
...
The idea is to choose one $(A,\tilde{b}) \in (A,\bullet)$ by fixing the weights. 
Because the stage derivatives are already known, we can choose a $(A,\tilde{b})$, that leads to a positive $u^{n+1}$, if $(A,\tilde{b}) \in (A,\bullet) | u^{n+1} \geq 0$ exists.
...
Because the coefficients in $A$ are already fixed, the Order Conditions reduce to linear equations.
This means that finding the weights can be done by solving a LP.
...
By incorporating the order conditions, the space of possible weights is limited to a subset of $\R^s$. %(Note: not linear because 0 is not in subspace)
