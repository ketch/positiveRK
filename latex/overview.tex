...
For $\dt \leq 0.0082$ the unaltered method lead to positive solutions. For larger $\dt$ the original method leads to negative values and $b$ is altered. For $\dt >0.016$ the dp5 is no longer stable.
...
These can be corrected by adapting the $b$. 
...
For both approaches the remaining steps can be computed with the standard $b$.

In figure\,\ref{fig:conv_impl} the convergence is shown. For $\dt < \num{3e-5} $ the standard $b$ yields to a positive result. For larger $\dt$ the $b$ have to be adapted to ensure positivity.
