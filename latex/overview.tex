...
When used with explicit methods 



Cost of solving the LP significant (Depending on complexity of RHS)
...
(Positifity of stagevaulues not esured, therefore the right had side has to be defined for negative values. If this is not ensured, the right hand side has to be defined for negative values.
%All the following testproblems did not nead this modification.

As first test problem we consider the PDE
\begin{equation}
...
\label{fig:conv_expl}
\end{figure}
TODO: Add converence bars and give exolaination
...
Also all stegevalues are computed using a the BE method. This means that all intermedaite stages are nonnegative. 
...
The siolutions are computed using the direct adaptation and convex combination for $\dt = \num{1e-3}$
...
But the method still outperformes the BE 
