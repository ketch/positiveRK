...
A third property is the existence of a positive solution. This corresponds to the question whether there is an embedded first order method that ensures positivity. This is particularly interesting for implicit methods, because the $\dt$ is not limited by the stability.
...
In Figure\,\ref{fig:stab} the stability regions of adapted RKMs are plotted.
...
\todo{Note (HR)} No. They are polynomials or rational functions
on the complex plane. Hence, they can only be bounded if they are
constant.
...
\begin{remark}
  ...
  If all the used embedded methods used to construct the new weights
...
$\| z (\I - z A)^{-1} e \|_\infty$ can be bounded by polynomial
in $|z|$.


\section{Implementation aspects of the algorithm}\label{sec:imple}

\todo{TODO}: Discuss in this Section aspects that are needed for implementation (and improve the usability) but do not concern the mathematical idea of the approach. %TODO
...
At fist the stage values $y_i$ and stage derivatives $f_i$ are calculated.
...
For this different orders are tried, starting with the highest order that has degrees of freedom for $\tilde{b}$.
At first the order conditions are constructed. On these an LP solver is invoiced. If the LP is infeasible the algorithm immediately reduces the order. If the LP is feasible we test if the solution is acceptable. At first it is tested whether the new $u^{n+1}$ indeed complies with the constraints. This is necessary because sometimes the used LP-solvers incorrectly identify the problem as feasible. %(Note: because of the chosen objective function it cannot be unbounded)
...
If the new set of weights is rejected in one of the tests the order is reduced.
...
\begin{equation}
\frac{\partial u}{\partial t} = -a \frac{\partial u}{\partial x} - K (u)
\end{equation}
...
\caption{Adaptation of the weights used for }
...
For large (\todo{TODO}: better word for 'not small enough') $\dt$ the method computes negative values for $u^1$.
