...
\end{equation}
These Polynomials are $\in \mathcal{L}^\infty$. (Note: for all explicit methods this is always true, for implicit methods we have to exclude the poles $\{z | \det(I - zA) = 0\}$ or  $\{z | \det(I - zA) > \epsilon \}$ )
...
The $P(z)$ is a linear combination of the Polynomials
\begin{subequations}
\begin{align}
...
The stability function $R_{\chi a + (1-\chi) c}(z)$ of a convex combination can be calculated by the weighted sum of the Stability functions $R_a(z)$ and $R_c(z)$

Proof:
\begin{multline}
R_{\chi a + (1-\chi) c}(z) = R(z) = 1 + (\chi a + (1-\chi) c)^Tz(I - zA)^{-1}e \\
= \chi + (1-\chi) + \chi a ^Tz(I - zA)^{-1}e + (1-\chi) c^Tz(I - zA)^{-1}e \\
= \chi R_a(z) + (1-\chi) R_c(z)
\end{multline}
...
\begin{multline}\label{eq:derivative_to_b}
 \frac{\partial f}{\partial \hat{b}_j}(w,a(w)) \Big|_{\hat{b}=0} = 
 \frac{\partial }{\partial \hat{b}}(R(\hat{b},z)R(\hat{b},z^*)) \\
 = (B\hat{b})^T (I-zA)^{-1} R_{b_{orig}}(z^*) + (B\hat{b})^T (I-z^*A)^{-1} R_{b_{orig}}(z)
\end{multline}
