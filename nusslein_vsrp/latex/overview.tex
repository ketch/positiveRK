...
\begin{align} \label{ode}
    ...
    u(0) & = u_0
\end{align}
where $u: [0,T] \to \R^m$ is positive if
...
problem or in reaction systems. Preserve of linear
...
different RKMs, how it can be combined adaptive error control, and how the
region of absolute stability can be approximated.
Section~\ref{sec:imple} contains some details regarding efficient implementation 
of the algorithm.
...
The difference $\|\bt-b\|$ is also plotted in Figure\,\ref{fig:exampleI}.
...
Our proposed method is summarized in Algorithm\,\ref{alg:Adaption}.
...
Note that for any diagonally implicit method, by allowing the order to be reduced to one
