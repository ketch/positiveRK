...
In this section an algorithm to update the solution $u^n$ using the adaptive RKM is proposed.
...
\begin{algorithm}
\begin{algorithmic}[1]
\State Caclculate Stagedrivatives
\State $u^{n+1}=\Delta t Fb$
\If {$u^{n+1} \geq 0$} 
        \State Update and End
\Else
	\ForAll{$\tilde p \in [p_{start}:p_{min}]$} 
		\State Solve LP
		\If {LP Feasible}
			\State caclulate $err_{adapt}$
			\If {$err_{adapt} < tol$}
				\State $u^{n+1}=\Delta t F \tilde b$
				\State Update and End
			\EndIf 
		\EndIf
	\EndFor
	\State Reject step and End
\EndIf 
\end{algorithmic}
...
\end{equation}
For problems where the exact solution is positive for certain $u_0$ but positivity is not preserved for all $u_0 \geq 0$ tests did not show promising results. Additionally, it is not certain whether the computed solutions would be reasonable.
